...
define("domanager-resources/editors/display/displayOptionsContentAreaEditor",
  [
    // Dojo
    "dojo/_base/declare",
    "dojo/_base/lang",
    ...
    return declare([
      ...
      postMixInProperties: function () {
        ...
        this.commands = this._commands || [
          new EditCommand(),
          new SelectDisplayOption(this.displayOptions),    // pass the displayoptions defined on the contentarea to the command via constructor
      ...
      buildRendering: function () {
        ...
        this.getCurrentContent().then(function(currentContent) {
          dataHelper.contentTypeSupported(currentContent.contentTypeID).then(function(isSupported) {
            if (isSupported === 'true') {
              this.actionsResource.emptyactions.template = 'You can drop content here, {createnewblock} or {selectatemplate}';
              this.actionsResource.emptyactions.actions = {
                createnewblock: "create a new block",
                selectatemplate: "select an existing template"
              }
            } else {
              this.actionsResource = blockAreaResources;
            }
      ...
      executeAction: function (actionName) {
        ...
        else if (actionName === "createnewblock") {
          // HACK: Preventing the onBlur from being executed so the editor wrapper keeps this editor in editing state
          this._preventOnBlur = true;
