define([
    ...
    function (
        ...
        return declare("score.editors.ColorPickerEditor", [_Widget, _TemplatedMixin, _WidgetsInTemplateMixin, _CssStateMixin, _ValueRequiredMixin], {
            ...
            _bindEvents: function (myself) {
                ...
                on(query(this.colorPickerList).query(".color-select"), "click", function (e) {
                ...
                on(query(this.colorPickerList).query(".color-clear"), "click", function (e) {
                    myself._setValue("", true);
                    myself._markChosenColorInList(null);
                    e.preventDefault();
                });
            ...
            _initColors: function () {
                ...
                array.forEach(this.selections, function (selection) {
                    domConstruct.create('a', { href: '#', title: selection.text, 'data-color': selection.value, style: 'background-color:' + selection.value + ';', class: "color-select" }
                        , domConstruct.create('li', null, colorPicker));
                });

                domConstruct.create('li',
                    { innerHTML: "<a href='#' class='color-clear'>X</a>" },
                    colorPicker);

                if (this.onPageEditMode) {
                    this.onPageEditMode = false;
                    this._markChosenColorInList(this.value);
                }
            }
        });
    });
