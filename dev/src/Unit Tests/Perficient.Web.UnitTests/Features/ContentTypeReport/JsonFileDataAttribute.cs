...
namespace Perficient.Web.UnitTests.Features.ContentTypeReport
{
    public class JsonFileDataAttribute : DataAttribute
    {
        ...
        public JsonFileDataAttribute(string filePath, string propertyName, Type resultType)
        {
            _filePath = filePath;
            _propertyName = propertyName;
            _resultType = resultType;
        ...
        public override IEnumerable<object[]> GetData(MethodInfo testMethod)
        {
            ...
            return _dataType == null ? GetonlyResults(data) : GetData(data);
        }

        private IEnumerable<object[]> GetonlyResults(string jsonData)
        {
            var objectList = new List<object[]>();
            var specific = typeof(NoResultsJsonFileHelperObject<>).MakeGenericType(_resultType);
        ...
        {
            var objectList = new List<object[]>();
            var specific = typeof(JsonFileHelperObject<,>).MakeGenericType(_dataType, _resultType);
            var generic = typeof(List<>).MakeGenericType(specific);
