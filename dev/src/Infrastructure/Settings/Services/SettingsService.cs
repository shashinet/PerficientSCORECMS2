...
namespace Perficient.Infrastructure.Settings.Services
{
    public class SettingsService : ISettingsService
    {
        ...
        public T GetSiteSettings<T>(CultureInfo preferredLanguage, Guid? siteId = null)
        {
            var contentLanguage = preferredLanguage.Name;
            if (!siteId.HasValue)
            {
            ...
            try
            {
                if (_contextModeResolver.CurrentMode == ContextMode.Edit)
                {
                    if (SiteSettings.TryGetValue(siteId.Value.ToString() + $"-common-draft-{contentLanguage}", out var siteSettings)
                        && siteSettings.TryGetValue(typeof(T), out var setting))
                    {
                        return (T)setting;
                    }

                    if (SiteSettings.TryGetValue(siteId.Value.ToString() + "-common-draft-default", out var defaultSiteSettings)
                        && defaultSiteSettings.TryGetValue(typeof(T), out var defaultSetting))
                    {
        ...
        {
            return GetSiteSettings<T>(ContentLanguage.PreferredCulture, siteId);
        }
        ...
        private Guid ResolveSiteId()
        {
            ...
            // Host.Host removes any ports from the host
            // Use Host.ToString() for local development
            var hostname = request.Host.Host.Contains("local")
                ? request.Host.ToString()
                : request.Host.Host;

            var site = _siteDefinitionResolver.GetByHostname(hostname, false, out _);
            if (site == null)
            {
                return Guid.Empty;
            }
